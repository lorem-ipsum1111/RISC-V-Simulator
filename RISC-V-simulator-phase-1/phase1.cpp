...
long long binConv(string binNum) // This function converts binary number in the form of string data-type into
{				// binary number in long - long data type
	long long decimal = 0;
...
int binaryToDecimal(long long n)  // This function takes long-long int data type binary number and converts it to
{ 				// decimal form with int data type
	int dec_value = 0; 
	// Initializing base value to 1, i.e 2^0 
	int base = 1; // We will use this variable as the place value of consecutive binary digits. We will keep
			//incrementing it
	long long  temp = n; 
	while (temp) { 
		long long  last_digit = temp % 10; // we keep extracting last digit from the binary figure
		temp = temp / 10; //and keep dividing it by 10 to truncate last digit
		dec_value += last_digit * base; //We then add last digit to decimal value after multiplying it by
						//appropriate power of 2
		base = base * 2; //Here we increase the power of the base by one
	} 
	return dec_value; 
} 
